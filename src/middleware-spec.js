...
describe('Exception', () => {
  describe('middleware', () => {
    ...
    app.get('/gateway-error', (req, res, next) => next(new BadGateway('No!', req)));

    app.get('/unknown-error', (req, res, next) => {
      next(new Error('unknown-error'));
    });

    it('forwards Errors to the default error handler', () => request(app)
      .get('/unknown-error')
      .set('x-request-id', 1)
    ...
    it('forwards unhandled Errors to the default error handler', () => request(app)
      .get('/unhandled-error')
      .set('x-request-id', 2)
      .set('accept', 'application/json')
      .expect(500, /unhandled error/));

    it('converts exceptions to json http responses', () => request(app)
      .get('/gateway-error')
      .set('x-request-id', 3)
      .set('accept', 'application/json')
      .expect(502, {
    ...
    it('converts exceptions to html http responses', () => request(app)
      .get('/gateway-error')
      .set('x-request-id', 4)
    ...
    it('adds the req property if it was not included in the constructor', () => request(app)
      .get('/missing-req')
      .set('x-request-id', 5)
      .set('accept', 'text/html')
      .expect(502, 'BadGateway: No!\nGET /missing-req\nREQUEST ID: 5'));
