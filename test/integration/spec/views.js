...
describe('Exception', () => {
  describe('middleware', () => {
    describe('when the app has a views engine', () => {
      it('uses the view for the error\'s status code', () => request(app)
        .get('/bad-request')
        .set('x-request-id', 1)
      ...
      describe('when no view can be found for the error', () => {
        it('falls back to the internal text serializer', () => request(app)
          .get('/gateway-error')
          .set('x-request-id', 2)
          .set('accept', 'text/html')
          .expect(502, 'BadGateway: No!\nGET /gateway-error\nREQUEST ID: 2'));

        it('falls back to the internal json serializer', () => request(app)
          .get('/gateway-error')
          .set('x-request-id', 3)
      ...
      describe('when the view contains an error', () => {
        it('renders the render error, not the original error', () => request(app)
